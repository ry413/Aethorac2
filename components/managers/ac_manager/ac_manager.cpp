...
void AirConBase::update_state(uint8_t state, uint8_t temps) {
    ...
    // 拔卡状态并且拔卡时不允许操作空调
    if (!get_alive() && !AirConManager::getInstance().remove_card_air_usable) { 
        return;
    }

    // 上报空调操作
    if (power == 0x00 && is_running.load() == true) {
        add_log_entry("air", uid, "关闭", "", true);        // 因为是用户操控温控器面板, 所以传true让它记录日志
    } else if (power == 0x01 && is_running.load() == false) {
        add_log_entry("air", uid, "打开", "", true);
    }

    if (mode.load() != bitsToMode(mode_bits)) {
        add_log_entry("air", uid, modeBitsToName(mode_bits), "", true);
    }

    if (fan_speed.load() != bitsToFanSpeed(fan_speed_bits)) {
        add_log_entry("air", uid, fanSpeedBitsToName(fan_speed_bits), "", true);
    }

    if (target_temp_val != target_temp.load()) {
        add_log_entry("air", uid, "调整至" + std::to_string(target_temp_val) + "度", "", true);
    }
...
void SinglePipeFCU::execute(std::string operation, std::string parameter, int action_group_id, bool should_log) {
    add_log_entry("air", uid, operation, parameter, should_log);
...
void SinglePipeFCU::power_off() {
    ...
    low_output->disconnect();
    mid_output->disconnect();
    high_output->disconnect();
    water1_output->disconnect();
}
...
void AirConManager::update_states(uint8_t state, uint8_t temps) {
    uint8_t id = state & 0x07;
    auto airCons = DeviceManager::getInstance().getDevicesOfType<AirConBase>();

    // 把state和temps传入每个id相同的空调
    for (auto& ac : airCons) {
        if (ac->ac_id == id) {
            ac->update_state(state, temps);
        }
    }
}

void AirConManager::update_room_temp(uint8_t ac_id, uint8_t temp) {
    auto airCons = DeviceManager::getInstance().getDevicesOfType<AirConBase>();
    for (auto& ac : airCons) {
        if (ac->ac_id == ac_id) {
            ac->update_room_temp(temp);
        }
    }
}


ACMode AirConBase::get_mode() { return mode.load(); }

ACFanSpeed AirConBase::get_fan_speed() { return fan_speed.load(); }

uint8_t AirConBase::get_target_temp() { return target_temp.load(); }

uint8_t AirConBase::get_room_temp() { return room_temp.load(); }

bool AirConBase::get_state() { return is_running.load(); }

void AirConBase::update_room_temp(uint8_t temp) {
    room_temp.store(temp);
}
